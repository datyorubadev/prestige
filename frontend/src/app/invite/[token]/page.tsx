...
function AcceptInvite() {
  ...
  const [info, setInfo] = useState<{ email: string; role: string; tenant: string; expiresAt: string } | null>(null);
  ...
  useEffect(() => {
    ...
    let active = true;
    api
      .get<{ email: string; role: string; tenant: string; expiresAt: string }>(
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      const result = await api.post<{ token: string }>("/auth/accept-invite", {
        invite_token: token,
        password,
    ...
    <InviteShell>
      {!token || invalid ? (
        <div className="flex flex-col items-center gap-3 py-2 text-center">
          ...
          <p className="text-[13.5px] font-semibold text-text">Invitation expired</p>
          <p className="max-w-[300px] text-[12.5px] leading-relaxed text-text-2">
            This invitation link is invalid or has already been used. Ask your team owner to send a new one.
          </p>
          <Link
            href="/login"
        ...
        <>
          <div className="mb-5 flex flex-col items-center text-center">
            ...
            <h1 className="text-[19px] font-extrabold tracking-tight text-text">Join {info?.tenant || "the team"}</h1>
            <p className="mt-1 text-[12.5px] text-text-2">
              You&apos;ve been invited as <span className="font-semibold text-text">{info?.role}</span>
            </p>
            <p className="text-[12px] text-text-3">{info?.email}</p>
          </div>
